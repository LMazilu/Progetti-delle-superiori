...
int ang1,ang2,ang3,ang4,orr,vert,X,Y,orr1,vert1;
...
int main() {
	...
	int caricato; 
	int incrementox; // variabile di stato del Tabellone: 0 --> non caricato
	//                                   1 --> caricato
	char x[LS]; //variabile per cognome nella ricerca sequenziale
	int li,ls,trovato,continuare,m;
	...
	do { // Inizio ciclo di ripetizione del programma
		...
		printf("\n\n 5 - Ricerca sequenziale");
		printf("\n\n 6 - Uscita dal programma");
		do {                            // Input della scelta...
			printf("\n\n\n Scelta --> ");
			scanf("%d", &scelta);
		} while(scelta<1||scelta>6);     // ...con controllo
		switch(scelta) {
			case 1: {
				cornice();
			...
			case 5: {
				...
				gets(x);
				for(incrementox=0; incrementox<LS; incrementox++) {
					x[incrementox]=toupper(x[incrementox])
				;};
				/* Ricerca */
				it=0;
				trovato=0;
				...
				getch();
				if(ordinato==0) {
					while(it<NA && trovato==0) {
						c=strcmp(x,Tabellone[it].co);
						if(c==0) {
							trovato = 1;
						...
						printf("it= %d ",it-1);
						puts(Tabellone[it-1].co);
						getch();
					}

				}
				if(ordinato==1) {
					for(it=0; it<=NA&&continuare==1; it++) {
						c=strcmp(x,Tabellone[it].co);
						if (c<0) {
							continuare=0;
						}
